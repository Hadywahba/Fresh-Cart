  ...
  return (
    <>
      ...
      <div className="container mx-auto flex-grow px-8 py-26 md:py-24">
        <h1 className="text-center text-3xl  sm:text-4xl font-bold text-main my-10 pb-7">
          All Brands
        </h1>
        {isLoading ? (
          <Loader />
          ...
          <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-9 my-8">
            {data?.map((brands) => (
              <div
                onClick={() =>
                  openModal(brands.image, brands.name, brands.slug)
                }
                key={brands._id}
                className=" border-2 border-gray-200  hover:scale-105 cart  "
              >
          ...
          <div className="content bg-white py-12 px-6 w-[700px] ">
            ...
            <div className="border-t-2 ">
              <button
                type="button"
                onClick={() => closeModal()}
                className="bg-main  w-full p-4 rounded-md my-5 right-0 "
              >
                Close
              </button>
            </div>
