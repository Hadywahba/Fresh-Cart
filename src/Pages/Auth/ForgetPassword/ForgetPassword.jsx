...
export default function ForgetPassword() {
  ...
  const loginformik = useFormik({
   ...
   localStorage.setItem("getemail",loginformik.values.email )

    
 console.log(loginformik.values.email)
  return (
      ...
      >
        <div className="relative z-0 w-full mb-5 group">
          ...
          <div className="flex justify-center mt-8">
            <button
              type="submit"
              className="bg-main  p-4 rounded-md text-white inline-block text-xl   "
            >
              {loading ? (
                <i className="fa-solid fa-spinner fa-spin text-center text-white"></i>
              ) : (
                <p className="">Send verify code</p>
              )}
            </button>
          </div>
        </div>
